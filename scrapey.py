        ...
        for entry in ap_list:
            ...
            if sn == entry["bssid"].upper():
                if rc not in client_list:
                    ...
                    os.system("echo '%s,%s' >> scope.txt" % (str(rc),(get_vendor_mac(rc))))
                ...
                if sn not in client_list:
                    ...
                    os.system("echo '%s,%s' >> scope.txt" % (str(sn),(get_vendor_mac(sn))))
...
def main():
    ...
    # Continually loop though AP's and find clients
    while True:
        channel_hop(args)
        find_clients(args)
